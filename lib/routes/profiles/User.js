...
module.exports = Router()
  ...
  .post('/', checkAuth(TEACHER), async(req, res, next) => {
    try {
      ...
      const auth0Id = await newAuth0User.user_id;
...
// .patch('/', checkAuth(TEACHER), async(req, res, next) => {
//   try {
//     const { auth0id, firstName, lastName, email, currentCourse, role } = req.body;
//     const user = await User
//       .create({ auth0id, firstName, lastName, email, role });
//     switch(role) {
//       case 'teacher':
...
//           });
//       case 'student':
//         return Student.create({ user: user._id, currentCourse })
//           .then(newStudent => {
//             res.send({ user, newStudent });
//           });
//     }
