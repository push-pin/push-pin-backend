...
module.exports = Router()
  .post('/', checkAuth(TEACHER, TA), (req, res, next) => {
    const { course, type, title, instructions, classDate, dateAvailable, dateDue, dateClosed } = req.body;
    ...
    Assignment
      .create({ course, type, title, instructions, classDate, dateAvailable, dateDue, dateClosed, pointsPossible })
  ...
  .get('/courseassignments/course/:course/student/:student', (req, res, next) => {
    const { course, student } = req.params;
    Promise.all([
      Assignment.find({ course }).lean(),
      Submission.courseSubs(student, course)
    ])
      .then(([asses, subs]) => res.send({ asses, subs }))
      .catch(next);
  })
